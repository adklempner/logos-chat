...
if [ "$SLIM" = "1" ]; then
    ...
    if [ -x "$LEZ_RLN_DIR/lez-rln/target/debug/run_setup" ]; then
        SETUP_OUTPUT=$(cd "$LEZ_RLN_DIR/lez-rln" && ./target/debug/run_setup 2>&1) || { echo "$SETUP_OUTPUT" | tail -30; die "run_setup failed"; }
    else
        SETUP_OUTPUT=$(cd "$LEZ_RLN_DIR/lez-rln" && cargo run --bin run_setup 2>&1) || { echo "$SETUP_OUTPUT" | tail -30; die "run_setup failed"; }
    fi
...
start_logoscore_instance "$RECEIVER_LOG" "$RECV_MDIR" "$CHAT_LOAD_ORDER" \
    ...
    "delivery_module.start()" \
    "delivery_module.subscribe($CONTENT_TOPIC)" \
    "chat_module.init_after_delivery($RECV_INSTANCE_DIR)" \
    "chat_module.get_address()" || true
...
SENDER_CALLS=(
    ...
    "delivery_module.start()"
    "delivery_module.subscribe($CONTENT_TOPIC)"
    "chat_module.init_after_delivery($SEND_INSTANCE_DIR)"
)
